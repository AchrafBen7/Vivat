...
class HomeArticlesSeeder extends Seeder
{
    /**
     * Crée les 10 articles de test pour GET /api/public/home.
     * Utilise les 9 catégories (par slug) et les mêmes slugs que la doc Postman.
     * À lancer après avoir créé les 9 catégories (Postman ou VivatNineCategoriesSeeder).
     */
    public function run(): void
    {
        $categoryBySlug = Category::query()->get()->keyBy('slug');
        $requiredSlugs = ['au-quotidien', 'energie', 'finance', 'technologie', 'chez-soi', 'mode', 'sante', 'voyage', 'famille'];
        foreach ($requiredSlugs as $slug) {
            if (! $categoryBySlug->has($slug)) {
                if ($this->command) {
                    $this->command->warn("Catégorie manquante : {$slug}. Crée les 9 catégories (voir docs/POSTMAN_9_CATEGORIES_BODIES.md).");
                }
        ...
        foreach ($articles as $data) {
            Article::where('slug', $data['slug'])->delete();
            $category = $categoryBySlug->get($data['category_slug']);
            ...
            $article->publish();
            $article->update(['published_at' => $data['published_at'] ?? now()->subDays(rand(0, 7))]);
        }

        $this->command?->info('10 articles home créés et publiés (slugs comme dans POSTMAN_10_ARTICLES_HOME_BODIES.md).');
    }

    /**
     * @return array<int, array{slug: string, category_slug: string, title: string, excerpt: string, content: string, meta_title: string, meta_description: string, reading_time: int, article_type: string, cover_image_url: ?string, keywords: string[], published_at?: \Carbon\Carbon}>
     */
...
HTML
            ...
            [
                ...
                'content' => <<<'HTML'
<p>L'Union européenne s'est fixé des objectifs ambitieux pour 2030 : réduction de 55 % des émissions de gaz à effet de serre par rapport à 1990, et 42,5 % d'énergies renouvelables dans la consommation finale. À mi-parcours, le bilan est contrasté selon les pays.</p>
...
HTML
            ...
            [
                ...
                'content' => <<<'HTML'
<p>Les prévisionnistes annoncent une dégradation nette du temps pour les prochains jours. Des précipitations neigeuses sont attendues en altitude dès ce soir, et des averses pluvieuses ou orageuses concerneront les plaines.</p>
...
HTML
            ...
            [
                ...
                'content' => <<<'HTML'
<p>Un rapport international publié ce mois-ci dresse un état des lieux préoccupant des écosystèmes marins. Les récifs coralliens, déjà menacés par le réchauffement et l'acidification des océans, subissent des épisodes de blanchissement de plus en plus fréquents et intenses.</p>
...
HTML
            ...
            [
                ...
                'published_at' => now(),
                'content' => <<<'HTML'
<p>La conférence des parties sur le climat s'est ouverte ce lundi avec en ligne de mire le respect des engagements de l'accord de Paris. Les pays sont attendus sur le renforcement de leurs contributions nationales et sur les financements en faveur des États les plus vulnérables.</p>
...
HTML
            ...
            [
                ...
                'content' => <<<'HTML'
<p>Bio, commerce équitable, FSC, MSC, Écolabel européen… Les étiquettes et logos se multiplient sur les produits. Ce guide vous aide à distinguer les labels les plus exigeants des simples arguments marketing.</p>
...
HTML
            ...
            [
                ...
                'content' => <<<'HTML'
<p>Le gouvernement a déposé un projet de loi visant à modifier les délais de prescription en matière civile et pénale. L'objectif affiché est de mieux prendre en compte la parole des victimes, notamment dans les affaires de violences ou d'abus, où le délai actuel est souvent jugé trop court.</p>
