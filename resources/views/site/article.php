...
<article class="max-w-3xl mx-auto">
    <header class="mb-8">
        ...
        <div class="flex gap-4 mt-4 text-sm text-gray-500">
            <?php if ($published_at): ?>
            <time datetime="<?= htmlspecialchars($published_at) ?>"><?= htmlspecialchars($published_at) ?></time>
            <?php endif; ?>
            <?php if ($reading_time): ?>
    ...
    <figure class="rounded-xl overflow-hidden mb-8">
        <img src="<?= htmlspecialchars($cover_image_url) ?>" alt="" class="w-full aspect-video object-cover">
    </figure>
    <?php endif; ?>
    <?php if ($excerpt): ?>
